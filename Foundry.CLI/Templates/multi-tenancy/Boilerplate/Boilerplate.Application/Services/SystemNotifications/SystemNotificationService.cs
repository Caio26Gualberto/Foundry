...
namespace Boilerplate.Application.Services.Notifications
{
    public class SystemNotificationService : ISystemNotificationService
    {
        ...
        public SystemNotificationService(IRepository<SystemNotification> repository, IRepository<SystemNotificationUser> repositoryNotificationsUsers,
            IRepository<User> userRepository, IRepository<Tenant> tenantRepository, IHubContext<SystemNotificationHub> hubContext,
        ...
        public async Task<Result<List<TenantNotificationDto>>> GetNotificationByTenant()
        {
            var tenantId = _currentUserContext.TenantId;

            var notifications = _repository
                .GetAll(x => x.SystemNotificationUser)
            ...
            var notificationsList = notifications
                .Select(n => new TenantNotificationDto
                {
                    ...
                    UsersCount = n.SystemNotificationUser.Count(),
                    ReadCount = n.SystemNotificationUser.Count(un => un.IsRead)
                })
                .OrderByDescending(n => n.CreatedAt)
                .ToList();

            return Result<List<TenantNotificationDto>>.Ok(notificationsList);
        }
